...
async def lancer(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if len(context.args) != 3:
        await update.message.reply_text("🛡️ Usage: /tabish <target_ip> <port> <duration>")
        return
    ...
    command = ['./RAJ', target_ip, str(port), str(duration)]
    try:
...
async def genkey(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if user_id in ADMIN_IDS:
        ...
        response = "ONLY OWNER CAN USE💀OWNER @HACK_CHIYE."
...
if __name__ == '__main__':
    ...
    app.add_handler(CommandHandler("tabish", lancer))
